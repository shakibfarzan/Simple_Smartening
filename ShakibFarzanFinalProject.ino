...
void onPump() {
  pumpTimer = 0;
  airState = onPump_onMotor;
}

void onPump_onMotor() {
  if (pumpTimer > 80) {
    digitalWrite(COOLER_MOTOR, HIGH);
    airState = onMotor;
  }
  pumpTimer++;
}
...
void onMotor() {
  byte currentTemp = getCurrentTemp();
  byte idealTemp = getIdealTemp();
  if ((currentTemp - idealTemp) >= 4) {
    digitalWrite(COOLER_SPEED, HIGH);
    airState = onSpeed;
  } else if (currentTemp < idealTemp) {
    digitalWrite(HEATER, HIGH);
    digitalWrite(WATER_PUMP, LOW);
...
void onSpeed(){
  byte currentTemp = getCurrentTemp();
  byte idealTemp = getIdealTemp();
  if ((currentTemp - idealTemp) < 4 && currentTemp > idealTemp) {
    digitalWrite(COOLER_SPEED, LOW);
    airState = onMotor;
  }
}
